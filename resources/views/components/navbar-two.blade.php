...
<nav {{ $attributes->merge(['class' => 'bg-primary shadow z-50 md:sticky md:top-0']) }}>
    <div class="py-2 sm:py-3 px-6 lg:px-28 container mx-auto flex justify-between items-center">
        ...
        <ul class="hidden md:flex space-x-4 lg:space-x-8 montserrat-bold text-md">
            <li><a href="{{ route('dashboard.overview', ['userId' => $userId]) }}" class="hover:text-secondary">Dashboard</a></li>
            <li><a href="" class="hover:text-secondary">Profile</a></li>
            <li><a href="{{ route('home') }}" class="py-2 px-8 bg-secondary text-dark rounded-full hover:bg-accent hover:text-primary duration-300 ease-in-out">Logout</a></li>
        </ul>
