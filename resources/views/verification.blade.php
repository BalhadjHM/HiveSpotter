    ...
    <x-slot name="content">
        <main class="h-full w-full bg-primary flex items-center justify-center overflow-hidden">
            <div class="w-[600px] py-12 px-8 bg-white rounded-lg shadow z-10 flex flex-col justify-center items-center text-center gap-8">
                ...
                <form action="{{ route('user.verify') }}" method="post" class="flex flex-col space-y-8">
                    <div>
                        @csrf
                        <label for="code" class="hidden"></label>
                        @for($i = 1; $i <= 6; $i++)
                            <input type="text" name="code[]" id="code" maxlength="1" class="w-8 sm:w-12 h-8 sm:h-12 text-center border-2 border-accent rounded-md focus:ring-0 focus:outline-0 montserrat-medium" required">
                        @endfor
                    </div>
                ...
                <a href="{{ route('user.store') }}" class="-mt-6 text-sm text-dark montserrat-medium duration-300 ease-out hover:text-secondary">Resend Verification Code</a>
            </div>
        </main>
    </x-slot>
