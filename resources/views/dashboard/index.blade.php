        ...
        <x-layout.dashboard>
            ...
            <div class="w-full flex flex-col justify-start items-stretch space-y-8">
                ...
                @if($trackers)
                    @foreach($trackers as $tracker)
                        <div id="tracker-card" class="w-full px-6 flex flex-col justify-center items-start bg-white border-slate-200 border p-4 rounded-lg shadow-md">
                            <div class="pb-4 w-full flex justify-between items-center">
                                <h2 class="text-2xl montserrat-semibold text-dark">{{ $tracker->name }}</h2>
                            ...
                            <div class="w-full flex flex-col md:flex-row justify-between items-start md:items-stretch gap-4">
                                <div class="md:basis-2/5 flex flex-col justify-between items-start space-y-4">
                                    <a href="https://{{ $tracker->domain }}/" class="md:pt-6 text-lg text-dark montserrat-medium border-b border-white duration-300 ease-out hover:text-accent hover:border-accent">{{ $tracker->url }}</a>
                                    <div class="pb-0 md:pb-8 grid grid-flow-col gap-2">
                                        <div>
                                            <p class="pb-1 text-sm sm:text-md text-dark montserrat-medium">Status</p>
                                            <div class="py-2 px-2 sm:px-4 text-center text-accent text-sm sm:text-xl border-2 border-accent montserrat-medium rounded-lg duration-300 ease-out hover:bg-accent hover:text-primary">{{ $tracker->status }}</div>
                                        </div>
                                        <div>
                                            <p class="pb-1 text-sm sm:text-md text-dark montserrat-medium">Uptime</p>
                                            <div class="py-2 px-2 sm:px-4 text-center text-accent text-sm sm:text-xl border-2 border-accent montserrat-medium rounded-lg duration-300 ease-out hover:bg-accent hover:text-primary">{{ $tracker->uptime }}%</div>
                                        </div>
                                        <div>
                                            <p class="pb-1 text-sm sm:text-md text-dark montserrat-medium">Speed</p>
                                            <div class="py-2 px-2 sm:px-4 text-center text-accent text-sm sm:text-xl border-2 border-accent montserrat-medium rounded-lg duration-300 ease-out hover:bg-accent hover:text-primary">{{ $tracker->speed }} s</div>
                                        </div>
                                    </div>
                    ...
                    @endforeach
                @else
                    <p class="text-dark montserrat-medium">No trackers found</p>
                @endif
            </div>
        </x-layout.dashboard>
    </x-slot>
